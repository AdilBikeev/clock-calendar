...
Если вы предпочитаете использовать серверный прокси (например, для дополнительной логики или мониторинга), вы можете настроить переменную окружения `VITE_TOKEN_PROXY_URL`. В этом случае приложение будет использовать прокси вместо прямого обмена через PKCE.
...
VITE_GOOGLE_CLIENT_ID=your_desktop_app_client_id_here
...
- Обязательно: `VITE_GOOGLE_CLIENT_ID` 
- Может потребоваться: `VITE_GOOGLE_CLIENT_SECRET` (если Google требует его для Desktop app в браузере)
...
- **Ошибка "Invalid client"**: Проверьте правильность `VITE_GOOGLE_CLIENT_ID` в `.env` файле
